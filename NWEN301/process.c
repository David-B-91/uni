...
process_execute (const char *file_name)
{
  ...
  /* =======================
     ...
     3. If successful, put the wait_stcmdatus of the child process into the children list.
     4. Otherwise, tid = TID_ERROR.
     ======================== */
  
  if (tid != TID_ERROR) {
      sema_down(&exec.load_done);
      if (exec.success) list_push_back(&thread_current()->children, &exec.wait_status->elem);
      else  tid = TID_ERROR;
    }
...
{
    int rc; // referecnce conut
    // acquire lock to access ref_cnt 
    lock_acquire(&cs->lock);
    rc = --cs->ref_cnt;
    lock_release(&cs->lock);
    // if child and parent are both dead, free cs.
    if(rc == 0) free(cs);
}
...
process_wait (tid_t child_tid)
{
  /* =======================
    ...
    // 1. Go through each child one by one.
    for (elem = list_begin (&thread_current()->children); elem != list_end (&current->children); elem = list_next(elem)) {
        struct wait_status *cur_wait_status = list_entry (elem, struct wait_status, elem);
        // 2. Check whether a child process is the one to be waited for.
        if (cur_wait_status->tid == child_tid) {
            //3. Wait for the child process to become dead.
            int exit_code;
            list_remove(elem);
            sema_down(&cur_wait_status->dead);
            // 4. Obtain exit code from the wait_status of the child process.
            exit_code = cur_wait_status->exit_code;
            // 5. Call release_child().
            release_child (cur_wait_status);
            //6. Return the exit code.
            return exit_code;
        }
    }
...
process_exit (void)
{
  ...
  {
      struct wait_status *cur_wait_status = cur->wait_status;
      printf ("%s: exit(%d)\n", cur->name, cur_wait_status->exit_code);
      // 1. Notify the parent process that this process is dead.
      sema_up(&cur_wait_status->dead);
      // 2. Release a reference to the wait_status of this process.
      release_child (cur_wait_status);
  }
  // 3. Go through the child list.
  for (elem = list_begin (&cur->children); elem != list_end (&cur->children); elem = next_elem) {
      struct wait_status *cur_wait_status = list_entry(elem, struct wait_status, elem);
      next_elem = list_remove(elem);
      // 4. Release a reference to the wait_status of each child process.
      release_child (cur_wait_status);
  }
