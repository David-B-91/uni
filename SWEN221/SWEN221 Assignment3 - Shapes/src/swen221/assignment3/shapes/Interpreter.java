...
public class Interpreter {
	...
	private void evaluateNextCommand(Canvas canvas) {
		...
		} /*else throw new IllegalArgumentException();//this makes syntax tests pass, but the rest fail. eh.*/
	}
	...
	public void fillShape(Color color, Shape shape, Canvas canvas) {
		...
		for (int x = 0 ; x < shape.boundingBox().width + shape.boundingBox().x ; x++){
			for (int y = 0 ; y < shape.boundingBox().height + shape.boundingBox().y ; y++){
		...
		for (int x = 0 ; x < shape.boundingBox().width + shape.boundingBox().x ; x++){
			for (int y = 0 ; y < shape.boundingBox().height + shape.boundingBox().y ; y++){
					if 	(shape.contains(x, y) && !shape.contains(x + 1, y) 
						||  shape.contains(x, y) && !shape.contains(x - 1, y) 
						||  shape.contains(x, y) && !shape.contains(x, y + 1) 
						||  shape.contains(x, y) && !shape.contains(x, y - 1)) {
					canvas.draw(x, y, color);
				}
			}
...
//		if (String.valueOf(input.charAt(index)).matches("^[()]$")) {
//			throw new IllegalArgumentException();
//		}
		int start = index;
		while (index < input.length()
