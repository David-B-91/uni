...
#Forwarding the Packet Recieved by the Controller
	...
	#TASK ONE METHOD, WORKS I THINK, PINGS DONT WORK BETWEEN h2 and h3
    ...
    def check_if_block(self, p_data):
        for protocol in p_data.protocols:
            if protocol.protocol_name == 'ipv4':
                if (protocol.src == h2_ipv4 and protocol.dst == h3_ipv4) or (protocol.src == h3_ipv4 and protocol.dst == h2_ipv4): 
                   return True
        return False

# TASK TWO METHODS -------------------------------------------------------------------------------
# From: https://osrg.github.io/ryu-book/en/html/traffic_monitor.html?fbclid=IwAR0HQ6nrK7mbXzANQeIvzMhSFbnhWrG_QvT6vrzCbnMDiLTv6fl9ep7h1Zk
    ...
    def _flow_stats_reply_handler(self, ev):
        global h1_in_count, h1_out_count, h1_total_count
#        body = ev.msg.body
#
...
# if traffic is in or out of h1, increment counts
#	    if stat.match['in_port'] == 1:
#	        h1_in_count = stat.packet_count
#	        h1_total_count = stat.packet_count
#	    elif stat.instructions[0].actions[0].port == 1:
#	        h1_out_count = stat.packet_count
#	        h1_total_count = stat.packet_count
...
#                         '-------- -------- --------')
        for stat in sorted(body, key=attrgetter('port_no')):
            ...
            if stat.port_no == 1:
                ...
                h1_in_count = stat.rx_packets
                h1_total_count = h1_out_count + h1_in_count

	#Print out h1 traffic information
	self.logger.info('------------------------------------------------------------')
	self.logger.info('h1_in_traffic: %s, h1_out_traffic: %s, h1_total_traffic: %s', h1_in_count, h1_out_count, h1_total_count)
	self.logger.info('------------------------------------------------------------')
