...
'''
    ...
    David's modification added support for more than two hosts and simultaneous send/receive of data.

    This version simplifies the sequence number strategy (Unique random number each packet) and 
    removes address field to make it independent of IP address version.

    Author: Jordan Ansell (jordan.ansell@ecs.vuw.ac.nz)
'''
...
class StopAndWaitHeader(PacketHeaderBase):
    ...
    def from_bytes(self, raw):
        ...
        self.msg_type = StopAndWaitType(fields[0])
        length = fields[1]
        ...
        return raw[StopAndWaitHeader._MINLEN + length:]
    ...
    def __str__(self):
        return 'StopAndWait type:{} seq:{} {}B payload'.format(
            self.msg_type.name, self.seq, len(self.payload) )

class StopAndWait():
    ''' 
        Modified StopAndWait transport protocol. A reliable transfer protocol.
    ...
    def get_src_address(self, dst_address):
        ...
        if type(dst_address) == IPv6Address:
            return self.inet6
        else:
            return self.inet4
    ...
    def handle_ack(self, packet, src_address):
        ...
        to_print = "ACK received from: {} seq: {}".format(src_address, packet.seq)
        ...
        if src_address != front_of_queue["dst"]:
            log_debug("{} (ignored - ACK from incorrect host".format(to_print))
            return
        if packet.seq < front_of_queue["seq"]:
            log_debug("{} (ignored - seq already acknowledged".format(to_print))
            return
    ...
    def handle_data(self, packet, src_address):
        ...
        log_debug("DATA received from: {} seq: {}  (Message received {}B)".format(
            src_address, packet.seq, len(packet)))

        self.app.accept_data(packet.payload, src_address)
    ...
    def accept_packet(self, packet_data, src_address):
        ...
        if not type(header) is StopAndWaitHeader:
            log_debug("Received Transport protocol which is not stop and wait, dropping")
            return

        header.from_bytes(raw=packet_data.to_bytes())

        if header.msg_type == StopAndWaitType.Ack:
            self.handle_ack(header, src_address)
        elif header.msg_type == StopAndWaitType.Data:
            self.handle_data(header, src_address)
        else:
