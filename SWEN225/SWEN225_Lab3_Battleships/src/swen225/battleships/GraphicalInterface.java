...
public class GraphicalInterface extends JFrame implements WindowListener {
    ...
    private void makeOutermostPanel(int width, int height) {
    	...
    	//initialize battleGrid squares

    	for (int i = 0; i<width ; i++) {
    		for (int j = 0 ; j < 0 ; j++) {
    			leftBattleGrid[i][j].setIcon(emptySquare);
    			rightBattleGrid[i][j].setIcon(emptySquare);
    		}
    	}
    	...
    	for (int i = 0; i<width ; i++) {
    		for (int j = 0 ; j < 0 ; j++) {
    			leftPanel.add(leftBattleGrid[i][j]);
    		}
    	}

    	outerMostPanel = new JPanel();
    	outerMostPanel.add(leftPanel);
    	outerMostPanel.add(new Button("new game"),FlowLayout.CENTER);
    	}
