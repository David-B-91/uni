...
void Scene::loadCore() {
	...
	m_boids.clear();
	for (int i = 0; i < 200; i++) {
		m_boids.push_back(Boid(linearRand(vec3(-m_bound_hsize), vec3(m_bound_hsize)), sphericalRand(1.0), 1));
	}
}
...
void Scene::loadCompletion() {
	...
	for (int i = 0; i < 100; i++) {
		m_boids.push_back(Boid(linearRand(vec3(-m_bound_hsize), vec3(m_bound_hsize)), sphericalRand(1.0), 1));
		m_boids.push_back(Boid(linearRand(vec3(-m_bound_hsize), vec3(m_bound_hsize)), sphericalRand(1.0), 2));
	}
	m_boids.push_back(Predator(linearRand(vec3(-m_bound_hsize), vec3(m_bound_hsize)), sphericalRand(1.0)));
}
...
void Scene::renderGUI() {
	...
	// YOUR CODE GOES HERE
	// ...
	ImGui::SliderFloat("MinSpeed", &minSpeed, 0, 100, "%.2f");
	ImGui::SliderFloat("MaxSpeed", &maxSpeed, minSpeed, 100, "%.2f", 2.0f);

	ImGui::Text("Confinement Type:");
	if (ImGui::Checkbox("Wrap", &confine_wrap)) {
		confine_bounce = false;
		confine_force = false;
	}
	ImGui::SameLine();
	if (ImGui::Checkbox("Bounce", &confine_bounce)) {
		confine_wrap = false;
		confine_force = false;
	}
	ImGui::SameLine();
	if (ImGui::Checkbox("Force", &confine_force)) {
		confine_wrap = false;
		confine_bounce = false;
	}
	ImGui::SliderFloat("Bounding Force", &bound_force, 0, 1.0, "%.3f");
	ImGui::TextColored(ImColor(255, 100, 100), "Boid Parameters:");
	ImGui::SliderFloat("Neighbourhood", &b_neighbourhood, 0, 100.0, "%.0f");
	ImGui::SliderFloat("Avoidance", &b_avoidance, 0, 1.0, "%.3f");
	ImGui::SliderFloat("Alignment", &b_alignment, 0, 1.0, "%.3f");
	ImGui::SliderFloat("Cohesion", &b_cohesion, 0, 1.0, "%.3f");
